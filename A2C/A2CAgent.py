...
class REINFORCEagent:
	def __init__(self, state_size, action_size):
		...
		self.Actor = self.Model.MLP(A2C_type='Actor')
		self.Critic = self.Model.MLP()
	...
	def act(self, state):
		...
		prediction = self.Actor(state).numpy()
	...
	def replay(self):
		...
		self.Actor.fit(state, actions, sample_weight = advantages, epochs = 1, verbose=0)
		self.Critic.fit(state, discounted_r, epochs = 1, verbose=0)
