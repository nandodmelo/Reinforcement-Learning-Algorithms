...
class DDQNagent:
    ...
    def model(self):
        ...
        v_network = Dense(64, activation="linear", kernel_initializer='he_uniform')(X)
        v_network = Dense(1, activation="linear", kernel_initializer='he_uniform')(v_network)

        v_advantage = Dense(64, activation="linear", kernel_initializer='he_uniform')(X)
        v_advantage = Dense(self.action_size, activation="linear", kernel_initializer='he_uniform')(v_advantage)
        
        #X = DuelingQ(v_advantage, v_network)

        advantage = Lambda(lambda a: a[:, :] - K.mean(a[:, :], keepdims=True), output_shape=(self.action_size,))(v_advantage)

        X = Add()([v_network, advantage])


        
        model = Model(inputs = X_input, outputs = X, name='Dueling_DDQN_model')
    ...
    def act(self, state):
        ...
        #print('actV', act_v, 'ACTv2', self.eval2(state).numpy())
